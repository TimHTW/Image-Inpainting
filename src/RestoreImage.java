...
public class RestoreImage {
	...
	public static void main(String[] args) throws IOException {
		...
		double[][] I_redScreen = new double [rows][columns];
		double[][] I_greenScreen = new double [rows][columns];
		double[][] I_blueScreen = new double [rows][columns];
		
		double[][] I_redScreen_ = new double [rows][columns];
		double[][] I_greenScreen_ = new double [rows][columns];
		double[][] I_blueScreen_ = new double [rows][columns];
		
		double[][] I_redOld = new double [rows][columns];
		double[][] I_greenOld = new double [rows][columns];
		double[][] I_blueOld = new double [rows][columns];
		
		
		
		boolean[][] paintedArea = new boolean [rows][columns];
		boolean[][] paintedArea_inner = new boolean[rows][columns];
		boolean[][] borderPixel = new boolean [rows][columns];
		
		boolean is_finish_inner;
		boolean is_finish_outer;
		

		double [][] Wred = new double [rows][columns];
		double [][] Wgreen = new double [rows][columns];
		double [][] Wblue = new double [rows][columns];
		double [][] Wred_ = new double [rows][columns];
		double [][] Wgreen_ = new double [rows][columns];
		double [][] Wblue_ = new double [rows][columns];
		
		
		double [][] Ix_red = new double [rows][columns];
		double [][] Iy_red = new double [rows][columns];
		double [][] Ix_green = new double [rows][columns];
		double [][] Iy_green = new double [rows][columns];
		double [][] Ix_blue = new double [rows][columns];
		double [][] Iy_blue = new double [rows][columns];
		double [][] Ix_rgb = new double [rows][columns];
		double [][] Iy_rgb = new double [rows][columns];
		
		double [][] Xscalar_red = new double [rows][columns];
		double [][] Xscalar_green = new double [rows][columns];
		double [][] Xscalar_blue = new double [rows][columns];
		
		
		double [][] Yscalar_red = new double [rows][columns];
		double [][] Yscalar_green = new double [rows][columns];
		double [][] Yscalar_blue = new double [rows][columns];
				...
				for(int i=0; i<rows; i++) {
					for(int j=0; j<columns; j++) {
						if(paintedArea[i][j]) {
							...
							/*--START Upwind--*/
							Iy_red[i][j] = (I_redScreen[i-1][j] - I_redScreen[i+1][j]) / 2.0;
							Ix_red[i][j] = -(I_redScreen[i][j+1] - I_redScreen[i][j-1]) / 2.0;
							Iy_green[i][j] = (I_greenScreen[i-1][j] - I_greenScreen[i+1][j]) / 2.0;
							Ix_green[i][j] = -(I_greenScreen[i][j+1] - I_greenScreen[i][j-1]) / 2.0;
							Iy_blue[i][j] = (I_blueScreen[i-1][j] - I_blueScreen[i+1][j]) / 2.0;
							Ix_blue[i][j] = -(I_blueScreen[i][j+1] - I_blueScreen[i][j-1]) / 2.0;
...
//							if(is_finish_outer) break outer_loop;
				...
				System.out.println(counter_border);
			}
		}
		...
		for(int i=0; i<rows; i++) {
			for(int j=0; j<columns; j++) {
				...
				resultImg.setRGB(j, i, newColor.getRGB());
			}
		}
		try {
